...
class ValueSelector(QWidget):
    ...
    def _browse_for_tag(self):
        preselected_tag = None
        if self.selected_tag_info:
            preselected_tag = self.selected_tag_info
            
        dialog = TagBrowserDialog(self, allowed_types=self.allowed_tag_types, allow_arrays=self.allow_arrays, preselected_tag=preselected_tag)
        if dialog.exec():
