...
class MainWindow(QMainWindow):
    ...
    def __init__(self, initial_project_path=None):
        ...
        # Create shared action group for all tools
        from PyQt6.QtGui import QActionGroup
        self.tool_action_group = QActionGroup(self)
        self.tool_action_group.setExclusive(True)
        self.tool_action_group.triggered.connect(lambda action: self.set_active_tool(action.data()))
        
        self.tools_toolbar = ToolsToolbar(self.tool_action_group, self)
        self.addToolBar(Qt.ToolBarArea.LeftToolBarArea, self.tools_toolbar)
        
        # Add drawing toolbar with drawing tools
        self.drawing_toolbar = DrawingToolbar(self.tool_action_group, self)
        ...
        # Connect dock floating state changes to update tab buttons visibility
    ...
    def _on_dock_top_level_changed(self, floating: bool):
        # Always show close buttons regardless of floating state
        if self.tab_widget.tabBar():
            self.tab_widget.tabBar().setTabsClosable(True)
        
        # Control dock widget features based on floating state
        sender = self.sender()
        if isinstance(sender, QDockWidget):
            # Ensure toggle action state matches visibility
            action = sender.toggleViewAction()
            action.setChecked(sender.isVisible())
            
            # Always enable all dock widget features regardless of floating state
            sender.setFeatures(
    ...
    def _connect_signals(self):
        ...
        self.ribbon.rect_action.triggered.connect(lambda: self.set_active_tool(constants.TOOL_RECTANGLE))
        self.ribbon.circle_action.triggered.connect(lambda: self.set_active_tool(constants.TOOL_CIRCLE))
        self.ribbon.polygon_action.triggered.connect(lambda: self.set_active_tool(constants.TOOL_POLYGON))
        self.ribbon.text_action.triggered.connect(lambda: self.set_active_tool(constants.TOOL_TEXT))
        
        # The tool_action_group handles tool changes now
